...
LOW_SENSORY <- 50
HIGH_SENSORY <- 50
TRIALS <- IMAGERY + LOW_SENSORY + HIGH_SENSORY
SUBJECTS <- 50
...
# mu calculated using the sensory power law described in Brascamp et al (2007)
# mu low sensory assumes equal low-intensity stimulus for all participants
# mu imagery we don't actually use in the model, just have it here as ever so slightly lower than lower sensory
# for each participant, we want to create some measure of imagery strength. this is going to be a covariate in the model
MU_IMAGERY <- 0.18^0.75 * 4
...
SD_IMAGERY <- 1.5 ## SD_HIGH_SENSORY is fixed to 1, low sensory items are more variable
MU_IMAGERY_STRENGTH <- 0.7 
SD_IMAGERY_STRENGTH <- 0.1 ## participant-level variation in LOW_SENSORY; .012*sqrt(238) if we wanna proxy by reinstatement
...
df <- data.frame(id=rep(1:SUBJECTS, each=TRIALS),
                 perceive=rep(rep(c(0,1,2), c(IMAGERY, LOW_SENSORY, HIGH_SENSORY)), SUBJECTS),
                 ...
                 imagery_strength=rep(rnorm(SUBJECTS, MU_IMAGERY_STRENGTH, SD_IMAGERY_STRENGTH), each=TRIALS)) %>%
...
df %>% data_grid(id, perceive, rating) %>%
    ...
    ungroup() %>%
    mutate(perceive=ifelse(perceive, 'Hits', 'FAs')) %>% ### pick up here ### 
...
m <- brm(bf(rating ~ perceive + (1 + perceive |i| id)), #intercepts = biases, coef for perceive = dprime
         ...
         prior=c(prior(normal(0, 1), class='b', coef='perceive')),
                 prior(normal(-MU_DPRIME, 1.5), class='b', coef='imagine', dpar='disc')),
